...
Cloud computing technology is  the availability of data over the internet.
...
The project uses Google Cloud Storage to save, convert and access the converted files, without the use of the local disk space.
The track of data is made possible through a database that records all the saved files, and thus we solve the problem of data duplication.
The service is available only from within a profile, therefore, in order for a user to use the service, the user has to create an account, and then authenticate with the created credentials.
...
With the spread of the event based architectures, the messaging systems have become the basic components of the enterprise architectures. Because the enterprise applications process more and more data, the performance of messaging systems and the need for rapid and scalable platforms have become of a paramount importance for the smooth running of the applications.
...
In Kafka there is a producer of messages and a consumer of those messages. The producer publishes messages in Kafka, that become available to cosumers for processing, but the consumers don’t have to confirm the processing. Kafka stores all the messages received in a fixed period of time, and the cosumers have the liberty to consume any of the stored messages. Because of this, this model (Kafka) has a series of advantages:
1)  It simplifies the architecture of the system
2)  Kafka doesn’t have to keep a record of which messages were consumed and which were not. It isolates the producers form the consumers. In the systems in which the consumers are obligated to confirm the processing of messages, the systems’s performance can decrease once the number of messages raises. Some services limit the rate at which the producers can publish messages, to prevent the overloading of the messaging system, but this can lead to dangerous situations where a slow but unimportant consumer affects a producer critical for business.
3) The consumers don’t have to constantly consume. Thus the consumers can be stopped anytime without affecting the messaging system. They can be even batch jobs periodically executed. But this approach works only if the messages are stored in Kafka long enough for the processes to process the data between sessions.
4) Because the messages don’t have to be selected when stored, Kafka can use a storage model called “messaging journal”. This journal is a messaging queue in which the new messages are always added at the end of the list. The existing messages never change. Because the journal is modified only at the end by adding new messages, it can be optimum stored on magnetic storage devices – the writing on the hard-disk can be done sequential thus avoiding the move of the starting point of the writing, which could have decreased the performance. If the consumers manage to keep up with the producers, Kafka can deliver messages directly from memory, using a caching mechanism offered by the operating system.

Another important difference from the traditional systems is the way the messages can be consumed.
The traditional systems offer 2 ways of consuming:
1. Queue
2. Publish-Subscribe
...
The index of the last processed message of every partition, is recorded in Zookeeper, therefore if a consumer leaves the group, that consumer’s partitions can be reasigned to other consumers, which can resume the process from the last message that was read. When a broker leaves the group, or becomes inaccessible, his partitions can become inaccessible or can be lost. To prevent these kind of situations,  Kafka has introduced the idea of “replicated partitions”. Each partitions that is hosted by a broker, can have one or more replications. The replications are partitions as well, but these partitions are hosted by different brokers than the original. Producers cannot publish messages in replications, only in the original (called leading partitions), and the brokers that host the original partition, make sure that every received message is saved in replications as well. In the event a broker is lost, one of the replications of every original partition of that broker takes the lead as the original partition. And thus, Kafka manages to work without interruptions and without loss of data. When the broker is resuming his activity, he syncronizes his partitions from the other brokers, and after that begins a process of assigning of leaders fro each partition.

* #### ZooKeeper

Kafka uses Zookeeper to detect craches, to implement the discovery of topics, and to maintain the production and the consumption state for topics. The design of Zookeeeper is specialized and very focused on coordination tasks.
...
Flask-Login is a small tool that specilizes in managing the aspect of a user authentication system, without being tied to a specific authentication mechanism.
...
1) It stores the active user’s ID in the session, and let you log them in and out easily.
...
SQLAlchemy has two major mode of usage, SQL Explession Language (reffered to as Core), and ORM. The modes can be used separately or together, depending on the application. Its major components are illustrated bellow, with component dependencies organized into layers:
![Image description](link-to-image)
...
The two modes use slightly different syntax, but the biggest difference betwee the two is the view of data as schema or business objects. SQLAlchemy Core has a schema-centric view, which follows the the traditional SQL being focused around tables, keys and index structures. SQLAlchemy Core’s strongest points are Data Warehouse, Reporting, Analysis, and where operating on unmodeled data and control the query is useful.
...
In order for the user to be able to run the application the user must first set the environment.
...
We must have Kafka server and Zookeeper running. For that we must execute the scripts that start the services, from the \bin\ directory of the two (Kafka and Zookeeper).

The user can start and stop and verify the status of Kafka and Zookeeper with the commands below:  
1) *kafka-server-start* (Windows) or *./kafka-server-start.sh* (Linux)
2) *kafka-server-stop* (Windows) or *./kafka-server-stop.sh* (Linux)
3) *zkServer.cmd start* (Windows) or *./zkServer.sh start* (Linux)
4) *zkServer.cmd stop* (Windows) or *./zkServer.sh stop* (Linux)
5) *zkServer.cmd status* (Windows) or *systemctl status zookeeper* (Linux)

### 3. User Access


In order to access the Flask Application, the user must have access to the server (e.g. http://127.0.0.1:5000).
The application is run from the Python IDE using the command “flask run” which is supposed to run in the directory the project is in.
